...
inline VehicleParameters::VehicleParameters() { }

inline VehicleParameters::VehicleParameters(ros::NodeHandle& nh) {
    load_from_parameter_workspace(nh);
}

inline void VehicleParameters::load_from_parameter_workspace(ros::NodeHandle& nh) {
    ...
    if (!nh.getParam(ros::this_node::getName() + "/maximum_road_wheel_angle", _maximum_road_wheel_angle))
        ROS_ERROR("%s: Could not get maximum_road_wheel_angle - using %f deg",
                  ros::this_node::getName().c_str(), VehicleModelHelpers::rad2deg(_maximum_road_wheel_angle));
    ...
    if (!nh.getParam(ros::this_node::getName() + "/maximum_steering_wheel_angle", _maximum_steering_wheel_angle))
        ROS_ERROR("%s: Could not get maximum_steering_wheel_angle - using %f deg",
                  ros::this_node::getName().c_str(), VehicleModelHelpers::rad2deg(_maximum_steering_wheel_angle));
...
inline float VehicleParameters::get_mass() {
...
inline float VehicleParameters::get_wheel_base() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
...
inline float VehicleParameters::get_yaw_inertia() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
    return _yaw_inertia;
...
inline float VehicleParameters::get_distance_front_axle() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
...
inline float VehicleParameters::get_distance_rear_axle() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
...
inline float VehicleParameters::get_distance_front_bumper() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
...
inline float VehicleParameters::get_distance_rear_bumper() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
...
inline float VehicleParameters::get_width() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
...
inline float VehicleParameters::get_height() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
    return _height;
...
inline float VehicleParameters::get_cornering_force_front() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
...
inline float VehicleParameters::get_conrering_force_rear() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
...
inline float VehicleParameters::get_max_rwa_deg() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
    return VehicleModelHelpers::rad2deg(_maximum_road_wheel_angle);
}

inline float VehicleParameters::get_max_swa_deg() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
    return VehicleModelHelpers::rad2deg(_maximum_steering_wheel_angle);
}

inline float VehicleParameters::get_max_rwa_rad() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
...
inline float VehicleParameters::get_max_swa_rad() {
    if (!_parametersLoadedFromWorkspace)
        ROS_ERROR_STREAM(ros::this_node::getName()
                         << ": Vehicle parameters not loades from Workspace, node is using default parameters!");
